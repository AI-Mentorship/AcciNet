...
def processCrash(crash, final_cells, numWithoutData, lock, i, crashes, apiCrashMapping, apiLock):
    if (crash["Latitude"], crash["Longitude"]) not in apiCrashMapping:
        (angle, lanes) = getExtraData(crash["Latitude"], crash["Longitude"])
        with apiLock:
            apiCrashMapping[(crash["Latitude"], crash["Longitude"])] = (angle, lanes)
    else:
        (angle, lanes) = apiCrashMapping[(crash["Latitude"], crash["Longitude"])]
...
def processCell(theCrashes):
    ...
    crashes = theCrashes.drop(columns=["index_right", "Crash ID", "geometry"])
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                raise Exception(f"One of the crashes failed, idk why. Error was {e}")
    return (f'In cell {cellIndex} we have {len(crashes)} crashes! New count also is {numWithoutData[0]}', final_cells, numWithoutData[0])