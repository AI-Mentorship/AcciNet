...
const AcciNetMap: React.FC<AcciNetMapProps> = ({ 
  ...
  const getDistance = (coord1 : number[], coord2 : number[]) => {
    const xDist = Math.abs(coord1[0] - coord2[0])
    const yDist = Math.abs(coord1[1] - coord2[1])

    const finalDistance = Math.sqrt(Math.pow(xDist, 2) + Math.pow(yDist, 2))

    return finalDistance
  }

  const getBestIndexMinimizingDistance = (routes : [number, number][][], bestIndices : number[], currentRouteIndex : number) => {
    let bestIndex = 0
    let bestIndexDistance = 0
    // loop through every coordinate found in the route
    for(let routePoints = 0; routePoints < routes[currentRouteIndex].length; routePoints++)
    {
      let smallestFoundDistance = Infinity;
      // loop through all other routes
      for (let otherRoutes = 0; otherRoutes < routes.length; otherRoutes++)
      {
        // except if it's the sameo ne
        if(currentRouteIndex === otherRoutes) continue;
        // loop through all the points in this current "other route"
        for (let otherRoutePoint = 0; otherRoutePoint < routes[otherRoutes].length;  otherRoutePoint ++)
        {
          // get distance between your current route's point and this other rout'es point
          const distance = getDistance(routes[currentRouteIndex][routePoints], routes[otherRoutes][otherRoutePoint])
          // 
          if(distance < smallestFoundDistance)
          {
            smallestFoundDistance = distance;
          }
        }
      }

      if(smallestFoundDistance > bestIndexDistance)
      {
        bestIndex = routePoints;
        bestIndexDistance = smallestFoundDistance
      }
    }
    return bestIndex
  }

  const idealMarkerPoints : [number, number][] = useMemo(() => {
    if (!mapLoaded || !mapRef.current || !routes.length) return [];
    
    // first, take only the middle portion of them
    const middleParts = routes.map(route => {
      return route.coords.slice(route.coords.length * 0.3, route.coords.length * 0.7)
    })

    const idealIndices = new Array(middleParts.length).fill(0)

    // run it twice because our intiial version may not return the ideal distance, since we start with bogus best indiex counts for the other routes
    for(let iterationsRun = 0; iterationsRun < 2; iterationsRun++)
    {
      for (let i = 0; i < middleParts.length; i++)
      {
        idealIndices[i] = getBestIndexMinimizingDistance(middleParts, idealIndices, i)
      }
    }

    return idealIndices.map((bestIndex : number, routeNum : number) => {
      return [middleParts[routeNum][bestIndex][0], middleParts[routeNum][bestIndex][1]]
    })
  }
  , [routes])
        ...
        {mapLoaded && mapRef.current && googleMapsLoaded && (
          ...
          {gradientRoutes.length > 0 && (
            <MultiRouteGradientLayer 
              ...
              placementPoints={idealMarkerPoints}
              fullRoutesData={routeInfos}
            />
          )}
