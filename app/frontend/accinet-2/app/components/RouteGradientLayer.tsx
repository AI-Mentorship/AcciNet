...
import BetterRouteSelector from './BetterRouteSelector';
...
type Props = {
  ...
  placementPoint: [number, number];
  fullRouteData: FullRouteInfo;
};

export default class RouteGradientLayer extends React.Component<Props> {
  private ids = { src: '', halo: '', grad: '' };
  private routeData : maplibregl.Marker | null = null;
  ...
  private ensure = () => {
    ...
    const routeBoxRoot = createRoot(routeBox);
    routeBoxRoot.render(<BetterRouteSelector route= {this.props.fullRouteData} />)

    const xMarker = document.createElement("div")
    const xMarkerRoot = createRoot(xMarker);
    xMarkerRoot.render(<div style={{
      width: "20px",
      height: "20px",
      background: "red",
      borderRadius: "50%",
    ...
    if (this.routeData && this.marker) 
    {
      this.routeData.remove();
      this.marker.remove();
    }

    this.routeData = new maplibregl.Marker({ element: routeBox }).setLngLat(this.props.placementPoint).addTo(map);
    this.marker = new maplibregl.Marker({element: xMarker}).setLngLat(this.props.placementPoint).addTo(map);
  };
